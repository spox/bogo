...
describe Bogo::Lazy do
  ...
  describe 'Clean behavior' do
    ...
    it 'should never be dirty' do
      ...
      instance.stringer.must_equal 'value'
      instance.dirty[:stringer].must_equal 'value'
      instance.data[:stringer].must_equal 'value'
      instance.attributes[:stringer].must_equal 'value'
      instance.dirty?(:stringer).must_equal false
    end

    it 'should reference same data structures' do
      instance.data.must_equal instance.dirty
      instance.dirty.must_equal instance.attributes
      [
        instance.data,
        instance.dirty,
        instance.attributes
      ].map(&:object_id).uniq.must_equal [instance.data.object_id]
    end
